...
def cal_idf_dict(phrase_bag_k, word_bag_k, varname_bag_v, descrip_v):
	...
	for phrase in phrase_bag_k:
		...
		for varname_items in varname_bag_v:
			if phrase in varname_items:
				ct += 1.0
				break
		if ct > 0:
			idf_phrase_varname[phrase] = math.log10(var_num/ct)
		else:
			idf_phrase_varname[phrase] = ct

		ct = 0.0
		for vardescrip_items in descrip_v:
			if phrase in vardescrip_items:
				ct += 1.0
		if ct > 0:
			idf_phrase_vardescrip[phrase] = math.log10(var_num/ct)
		else:
	...
	for word in word_bag_k:
		ct = 0.0
		for varname_items in varname_bag_v:
			if word in varname_items:
				ct += 1.0
				break
		if ct > 0:
			idf_word_varname[word] = math.log10(var_num/ct)
		else:
			idf_word_varname[word] = ct

		ct = 0.0
		for vardescrip_items in descrip_v:
			if word in vardescrip_items:
				ct += 1.0
		if ct > 0:
			idf_word_vardescrip[word] = math.log10(var_num/ct)
		else:
